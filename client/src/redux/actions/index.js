...
export const getDogs = () => (dispatch) => {
  return fetch(`${URL_API}/dog`)
    .then((r) => r.json())
    .then((data) => dispatch({ type: GET_DOGS, payload: data }));
};

export async function createNewBreed(payload) {
  let newBreed = await axios.post(`${URL_API}/dog`, payload);
  return newBreed;
}

export function createDog(payload) {
  return async function (dispatch) {
    const resApi = await axios.post(`${URL_API}/dog`, payload);
    return resApi;
  };
}
...
export const getDogByName = (dogName) => (dispatch) => {
  return fetch(`${URL_API}/dog?name=${dogName}`)
    .then((r) => r.json())
    .then((data) =>
      dispatch({
